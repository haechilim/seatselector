...
public class Main {
        ...
        for(int round = 0; round < TOTAL_ROUND; round++) {
            ...
            for (int i = 0; i < students.length; i++) {
                ...
                for (int j = i + 1; j < students.length; j++) {
                    ...
                    if (student.getMale() == candidate.getMale()) continue;   // 동성인지 확인
    ...
    private static void print(Student[] students, int theNumberOfTimes) {
        for(int i = 0; i < students.length; i++) {
            System.out.println(students[i].getId() + "  " + students[i].getMale() + "  " + students[i].getPartnerId(theNumberOfTimes));
        }
